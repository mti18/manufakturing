...
Number.prototype.rupiah = function (useComma = false) {
  if (this.toString().substr(0, 1) == "-") {
    if (useComma) {
      ...
      var val = this.toString().replace(/-/, "").replace(/\D/g, "");
      if (nml[1] != undefined && nml[0][1] != undefined) {
        ...
        if (
          parseInt(this.toString().replace(/\./g, "").replace(/\,/g, ".")) > 0
        ) {
          ...
          var str =
            nml[0] == undefined
              ? "0," + nml[1]
              : parseInt(this.toString().replace(/\,/g, ""));
        }
      } else {
        var str = val
          .toString()
    ...
    if (useComma) {
      var nml = this.toString().split(",");
      if (nml[1] != undefined && nml[0][1] != undefined) {
        var spl = this.toString().replace(/,/, "").replace(/\D/g, "");
        var str =
          this.toString()
            .replace(/,/, "")
            .replace(/\D/g, "")
