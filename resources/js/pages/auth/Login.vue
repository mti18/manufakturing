<template>
  ...
  >
    <div class="text-center mb-11">
      ...
      <div class="text-gray-500 fw-semibold fs-6">{{$page.props.setting.name}}</div>
    </div>
    ...
    <div v-if="auth.type === 'email'" class="fv-row mb-3 fv-plugins-icon-container">
    ...
    <div v-else-if="auth.type === 'nip'" class="fv-row mb-3 fv-plugins-icon-container">
      <label for="identity" class="form-label">Password</label>
      <input
        type="password"
        placeholder="Password"
    ...
    <div v-else-if="auth.type === 'phone'" class="fv-row mb-3 fv-plugins-icon-container">
    ...
    <div class="d-grid mb-10 mt-10">
      <button type="submit" class="btn btn-primary" :disabled="status === 'loading'" :data-kt-indicator="status === 'loading' ? 'on' : 'off'">
...
export default {
  ...
  methods: {
    ...
    login() {
      ...
      vm.axios.post('/auth/login', data).then(res => {
    ...
    checkCredential() {
      const vm = this;
      vm.status = "loading";
      vm.axios.post('/secure/check-credential', {
    ...
    getOtp() {
      const vm = this;
      vm.status = "loading";
      vm.axios.post('/secure/get-otp', { phone: vm.auth.phone }).then(res => {
        vm.status = 'success';
