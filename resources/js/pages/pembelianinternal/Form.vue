<template>
  <form class="card mb-12" id="form-pembelianinternal" @submit.prevent="onSubmit">
...
export default {
  ...
  setup(props) {
    ...
    const { data: pembelianinternal = {details: [], barangjadi: [], barangmentah: []}, refetch } = useQuery(
      ["pembelianinternal", selected, "edit"],
      () => {
        setTimeout(() => KTApp.block("#form-pembelianinternal"), 10);
      ...
      },
      {
        ...
        cacheTime: 0,
        onSuccess: (data) => {
          KTApp.unblock('#form-pembelianinternal')
          const datas = {...data};
    ...
    const { mutate: submit } = useMutation((data) => axios.post(selected.value ? `/pembelianinternal/${selected.value}/update` : '/pembelianinternal/store', data).then(res => res.data), {
      onMutate: () => {
        KTApp.block("#form-pembelianinternal");
      },
      onError: (error) => {
        toastr.error(error);
        KTApp.unblock("#form-pembelianinternal");
  ...
  methods: {
    ...
    onSubmit() {
      ...
      // const data = new FormData(document.getElementById("form-pembelianinternal"));
      this.submit(vm.form, {
        onSuccess: (data) => {
    ...
    gettahun() {
      this.$http
        .get("pembelianinternal/gettahun")
        .then((res) => {
    ...
    getbulan() {
      this.$http
        .get("pembelianinternal/getbulan")
        .then((res) => {
          console.log(res.data)
          this.bulan = res.data;
        })
    ...
    getnomor() {
      this.$http
        .get("pembelianinternal/getnomor")
        .then((res) => {
          console.log(res.data)
          this.nomor = res.data;
        })
    ...
    editGetnomor() {
      this.$http
        .get("pembelianinternal/" + this.selected + "/getnomor")
        .then((res) => {
          console.log(res.data);
          this.nomor = res.data;
        })
