<template>
    <section>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <mti-paginate id="table-position" url="/assetgroup/paginate" :columns="columns"></mti-paginate>
        </div>
      </div>
    </section>
  </template>
  ...
  export default {
    ...
    setup() {
      ...
      const { delete: deleteassetgroup } = useDelete({
        onSuccess: () => {
          queryClient.invalidateQueries(["/assetgroup/paginate"]);
        }
      })
      ...
      const columns = [
        ...
        columnHelper.accessor("nm_asset_group", {
          header: "Nama",
          cell: (cell) => cell.getValue(),
        }),
        ...
        columnHelper.accessor("uuid", {
          ...
          cell: (cell) => openForm.value ? null : h('div', { class: 'd-flex gap-2' }, [
              ...
              h('button', { class: 'btn btn-sm btn-icon btn-danger', onClick: () => {
                deleteassetgroup(`/assetgroup/${cell.getValue()}/destroy`);
