<template>
  <form class="card mb-12" id="form-stokkeluar" @submit.prevent="onSubmit">
    <div class="card-header">
      <div class="card-title w-100">
        <h3>
          {{
            stokkeluar?.uuid
              ? `Edit Stok Keluar : ${stokkeluar.name}`
              : "Tambah Stok Keluar"
          }}
        </h3>
        <button
    ...
    <div class="card-body">
      ...
      <div class="row">
        <div class="col-6">
          <div class="mb-8">
            <label for="barang_keluar" class="form-label required">
            ...
            <input
              ...
              name="barang_keluar"
              id="barang_keluar"
              placeholder="Jumlah Stok Barang Keluar"
              class="form-control"
              required
              autoComplete="off"
              v-model="form.barang_keluar"
            />
          </div>
        </div>
        ...
        <div class="col-6">
          <div class="mb-8">
            <label for="tanggal_keluar" class="form-label required">
              Tanggal :
            </label>
            <input
              type="date"
...
export default {
  ...
  setup({ selected }) {
    ...
    const { data: stokkeluar } = useQuery(
      ["stokkeluar", selected, "edit"],
      () => {
        setTimeout(() => KTApp.block("#form-stokkeluar"), 100);
        return axios
          .get(`/stokkeluar/${selected}/edit`)
          .then((res) => res.data);
      },
      {
        enabled: !!selected,
        ...
        onSettled: () => KTApp.unblock("#form-stokkeluar"),
      }
    );
    ...
    const { mutate: submit } = useMutation(
      (data) =>
        axios
          .post(
            selected ? `/stokkeluar/${selected}/update` : "/stokkeluar/store",
            data
          )
          .then((res) => res.data),
      {
        onMutate: () => {
          KTApp.block("#form-stokkeluar");
        },
        onError: (error) => {
          toastr.error(error.response.data.message);
        },
        onSettled: () => {
          KTApp.unblock("#form-stokkeluar");
        },
      }
    );
  ...
  methods: {
    ...
    onSubmit() {
      const vm = this;
      const data = new FormData(document.getElementById("form-stokkeluar"));
      this.submit(data, {
        onSuccess: (data) => {
          toastr.success(data.message);
          vm.$parent.openForm = false;
          vm.$parent.selected = undefined;
          vm.queryClient.invalidateQueries(["/stokkeluar/paginate"], {
            exact: true,
          });
        },
      });
