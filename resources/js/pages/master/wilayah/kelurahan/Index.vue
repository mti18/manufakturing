<template>
    <section>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <mti-paginate id="table-kelurahan" url="/kelurahan/paginate" :columns="columns"></mti-paginate>
        </div>
      </div>
    </section>
  </template>
  ...
  export default {
    ...
    setup() {
      ...
      const { delete: deletekelurahan } = useDelete({
        onSuccess: () => {
          queryClient.invalidateQueries(["/kelurahan/paginate"]);
        }
      })
      ...
      const columns = [
        ...
        columnHelper.accessor("nm_kelurahan", {
          header: "Nama Kelurahan",
          cell: (cell) => cell.getValue(),
        }),
        columnHelper.accessor("namakec", {
          header: "Kecamatan",
          cell: (cell) => cell.getValue(),
        }),
        columnHelper.accessor("uuid", {
          ...
          cell: (cell) => openForm.value ? null : h('div', { class: 'd-flex gap-2' }, [
              ...
              h('button', { class: 'btn btn-sm btn-icon btn-danger', onClick: () => {
                deletekelurahan(`/kelurahan/${cell.getValue()}/destroy`);
