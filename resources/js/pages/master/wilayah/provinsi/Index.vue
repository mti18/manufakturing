<template>
    <section>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <mti-paginate id="table-provinsi" url="/provinsi/paginate" :columns="columns"></mti-paginate>
        </div>
      </div>
    </section>
  </template>
  ...
  export default {
    ...
    setup() {
      ...
      const { delete: deleteprovinsi } = useDelete({
        onSuccess: () => {
          queryClient.invalidateQueries(["/provinsi/paginate"]);
        }
      })
      ...
      const columns = [
        ...
        columnHelper.accessor("nm_provinsi", {
          header: "Nama",
          cell: (cell) => cell.getValue(),
        }),
        columnHelper.accessor("uuid", {
          ...
          cell: (cell) => openForm.value ? null : h('div', { class: 'd-flex gap-2' }, [
              ...
              h('button', { class: 'btn btn-sm btn-icon btn-danger', onClick: () => {
                deleteprovinsi(`/provinsi/${cell.getValue()}/destroy`);
