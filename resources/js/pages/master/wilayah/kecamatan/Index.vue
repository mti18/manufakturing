<template>
    <section>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <mti-paginate id="table-kecamatan" url="/kecamatan/paginate" :columns="columns"></mti-paginate>
        </div>
      </div>
    </section>
  </template>
  ...
  export default {
    ...
    setup() {
      ...
      const { delete: deletekecamatan } = useDelete({
        onSuccess: () => {
          queryClient.invalidateQueries(["/kecamatan/paginate"]);
        }
      })
      ...
      const columns = [
        ...
        columnHelper.accessor("nm_kecamatan", {
        ...
        columnHelper.accessor("uuid", {
          ...
          cell: (cell) => openForm.value ? null : h('div', { class: 'd-flex gap-2' }, [
              ...
              h('button', { class: 'btn btn-sm btn-icon btn-danger', onClick: () => {
                deletekecamatan(`/kecamatan/${cell.getValue()}/destroy`);
