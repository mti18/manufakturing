<template>
    <form class="card mb-12" id="form-kecamatan" @submit.prevent="onSubmit">
      <div class="card-header">
        <div class="card-title w-100">
          <h3>
            {{ kecamatan?.uuid ? `Edit Kecamatan : ${kecamatan.name}` : "Tambah Kecamatan"  }}
          </h3>
          <button
            type="button"
      ...
      <div class="card-body">
        <div class="row">
          <div class="col-6">
            <div class="mb-8">
              <label for="name" class="form-label required"> Nama Kecamatan: </label>
              <input type="text" name="nm_kecamatan" id="nm_kecamatan" placeholder="Nama"
                class="form-control" required autoComplete="off" v-model="form.nm_kecamatan" />
            </div>
            <div class="mb-8">
  ...
  export default {
    ...
    setup({ selected }) {
      ...
      const { mutate: submit } = useMutation((data) => axios.post(selected ? `/kecamatan/${selected}/update` : '/kecamatan/store', data).then(res => res.data), {
        onMutate: () => {
      ...
      onSubmit() {
        ...
        this.submit(data, {
          onSuccess: (data) => {
            ...
            vm.queryClient.invalidateQueries(["/kecamatan/paginate"], { exact: true });
          }
        });
      }
    }
