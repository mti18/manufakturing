<template>
    <section>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <mti-paginate id="table-kota" url="/kota/paginate" :columns="columns"></mti-paginate>
        </div>
      </div>
    </section>
  </template>
  ...
  export default {
    ...
    setup() {
      ...
      const { delete: deletekota } = useDelete({
        onSuccess: () => {
          queryClient.invalidateQueries(["/kota/paginate"]);
        }
      })
      ...
      const columns = [
        ...
        columnHelper.accessor("nm_kab_kota", {
          header: "Nama Kota/Kabupaten",
          cell: (cell) => cell.getValue(),
        }),
        columnHelper.accessor("namaprov", {
          header: "Provinsi",
          cell: (cell) => cell.getValue(),
        }),
        columnHelper.accessor("uuid", {
          ...
          cell: (cell) => openForm.value ? null : h('div', { class: 'd-flex gap-2' }, [
              ...
              h('button', { class: 'btn btn-sm btn-icon btn-danger', onClick: () => {
                deletekota(`/kota/${cell.getValue()}/destroy`);
